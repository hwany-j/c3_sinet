...
parser.add_argument('--savedir', default='./Testserver', help='directory to save the results')
...
# os.environ["CUDA_VISIBLE_DEVICES"] = "1"
Test_folder = ["berlin", "bielefeld", "bonn", "leverkusen", "mainz", "munich"]
pallete = [[128, 64, 128],
           [244, 35, 232],
           [70, 70, 70],
           [102, 102, 156],
...
def evaluateModel(args, model, image_list, folder_name, inputsize):
    ...
    if not os.path.exists(os.path.join(args.savedir,'Test',folder_name)):
        os.mkdir(os.path.join(args.savedir, 'Test',folder_name))
    ...
    for i, imgName in enumerate(image_list):
        ...
        cv2.imwrite(args.savedir + os.sep + 'Test' + os.sep + folder_name + os.sep + name.replace(args.img_extn, 'png'), classMap_numpy)
...
def Vis_Result(modelname, config, use_nsml, max_name=None, mode="CV"):
    ############## save setting
    ...
    if not os.path.isdir(savedir):
        ...
        os.mkdir(os.path.join(savedir, 'Test'))
    if use_nsml:
        from nsml import DATASET_PATH
        data_dir = os.path.join(DATASET_PATH, 'train')

    args.config = config
    args.data_dir = os.path.join(data_dir,'leftImg8bit/test')
    ...
    if modelname.startswith('Dnc_C3'):
        model = models.__dict__[modelname](
            classes=train_config["num_classes"], p=train_config["p"], q=train_config["q"],
            D_rate=train_config["D_rate"], chnn=train_config["chnn"])

    elif modelname.startswith('Dnc_SIN'):
        model = models.__dict__[modelname](
            classes=train_config["num_classes"], p=train_config["p"], q=train_config["q"], chnn=train_config["chnn"])



    if os.path.isfile(max_name):
        model.load_state_dict(torch.load(max_name))
        print("Load maxfile : " + str(max_name))
    ...
    for i in range(len(Test_folder)):
        print("Test : {}".format(Test_folder[i]))
        full_name = os.path.join(args.data_dir , Test_folder[i])
        image_list = glob.glob(full_name +os.sep + '*.' + args.img_extn)
        print(len(image_list))
        # set to evaluation mode
        model.eval()
        if mode == "CV":
            evaluateModel(args, model, image_list, Test_folder[i], data_config["baseSize"])
        else:
            PILevaluateModel(args, model, image_list, Test_folder[i], data_config["baseSize"])
