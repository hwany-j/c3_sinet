...
class C3Net_Encoder(nn.Module):
    ...
    def __init__(self, classes=20, p=5, q=3, D_rate=[2,4,8,16], chnn=1.0):
        ...
        self.level1 = CBR(3, classes, 3, 2)
        self.sample1 = InputProjectionA(1)
        self.sample2 = InputProjectionA(2)

        self.b1 = BR(classes + 3)
        self.level2_0 = Down_C3module(classes + 3, int(64*chnn),D_rate=D_rate)
...
class C3Net(nn.Module):
    ...
    def __init__(self, classes=20, p=2, q=3, D_rate=[2,4,8,16], chnn=1.0, encoderFile=None,):
        ...
        self.bn = nn.BatchNorm2d(classes, eps=1e-03)
        self.up = nn.UpsamplingBilinear2d(scale_factor=2)

        self.conv_l2 = separableCBR(classes + int(64*chnn), classes, 3, 1)
        self.conv_l1 = separableCBR(classes + classes, classes, 3, 1)

        self.classifier = nn.Sequential(
            nn.UpsamplingBilinear2d(scale_factor=2),
            C(classes, classes, 1, 1)
        )
    ...
    def forward(self, input):
        ...
        Dnc2_0 = self.up(self.bn(self.encoder.classifier(output2_cat)))
        Dnc2_cat = self.conv_l2(torch.cat([Dnc2_0, output1], 1))

        Dnc1_0 = self.up(Dnc2_cat)
        Dnc1_cat = self.conv_l1(torch.cat([Dnc1_0, output0], 1))

        classifier = self.classifier(Dnc1_cat)
        return classifier



def Enc_C3Netv2(**kwargs):
    print("C3 Net Enc :  ")
    model = C3Net_Encoder(**kwargs)
    return model

def Dnc_C3Netv2(**kwargs):
    print("Full CCC netDnc")
    model = C3Net(**kwargs)
    return model
